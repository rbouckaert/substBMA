package substbma.math.matrixAlgebra1;

import cern.colt.matrix.DoubleFactory2D;
import cern.colt.matrix.DoubleMatrix2D;
import cern.colt.matrix.linalg.Property;
import substbma.math.util.MathUtil;
...
//            this.maxIterations = maxIterations;
              ...
              for (int k = 0; k < Math.max(nct,nrt); k++) {
                         if (k < nct) {
                                ...
                                s[k] = 0;
                                for (int i = k; i < m; i++) {
                                   s[k] = substbma.math.util.MathUtil.hypot(s[k], A[i][k]);
                                }
                                if (s[k] != 0.0) {
                                   if (A[k][k] < 0.0) {
                                              s[k] = -s[k];
                         ...
                         if (k < nrt) {
                                ...
                                e[k] = 0;
                                for (int i = k+1; i < n; i++) {
                                   e[k] = substbma.math.util.MathUtil.hypot(e[k], e[i]);
                                }
                                if (e[k] != 0.0) {
                                   if (e[k+1] < 0.0) {
                                              e[k] = -e[k];
